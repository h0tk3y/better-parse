...
class TokenTest {
    ...
    @Test fun successfulParse() {
        ...
        Assert.assertEquals(a, result.value.type)
        Assert.assertEquals(listOf(a, a), result.remainder.toList().map { it.type })
    }
    ...
    @Test fun unexpectedEof() {
        ...
        Assert.assertEquals(UnexpectedEof(a), result)
    }
    ...
    @Test fun noMatchingToken() {
        ...
        Assert.assertEquals(NoMatchingToken(TokenMatch(noneMatched, "c", 0, 1, 1)), result)
    }
    ...
    @Test fun ignored() {
        ...
        Assert.assertEquals(TokenMatch(a, "a", 3, 1, 4), result.toParsedOrThrow().value)
    }
    ...
    @Test fun mismatched() {
        ...
        Assert.assertEquals(MismatchedToken(a, TokenMatch(b, "b", 0, 1, 1)), result)
    }
