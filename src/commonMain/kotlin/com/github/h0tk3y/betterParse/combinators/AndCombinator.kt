...
public class AndCombinator<out R> @PublishedApi internal constructor(
    ...
    public val transform: (List<Any?>) -> R
) : Parser<R> {
    ...
    public override fun tryParse(tokens: TokenMatchesSequence, fromPosition: Int): ParseResult<R> {
        ...
        loop@ for (element in consumers) {
            when (element) {
                is Parser<*> -> when (val result = element.tryParse(tokens, nextPosition)) {
                ...
                is SkipParser -> when (val result = element.innerParser.tryParse(tokens, nextPosition)) {
                    is ErrorResult -> return result
