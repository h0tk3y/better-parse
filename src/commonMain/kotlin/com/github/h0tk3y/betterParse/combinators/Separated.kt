...
public class SeparatedCombinator<T, S>(
    ...
    override fun tryParse(tokens: TokenMatchesSequence, fromPosition: Int): ParseResult<Separated<T, S>> {
        ...
        return when (val first = termParser.tryParse(tokens, fromPosition)) {
            ...
            is Parsed -> {
                ...
                loop@ while (true)
                    when (val separator = separatorParser.tryParse(tokens, nextPosition)) {
                        is ErrorResult -> break@loop
                        is Parsed -> {
                            when (val nextTerm = termParser.tryParse(tokens, separator.nextPosition)) {
                                is ErrorResult -> break@loop
                                is Parsed -> {
