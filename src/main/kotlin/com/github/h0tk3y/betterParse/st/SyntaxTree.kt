...
fun <T> Parser<SyntaxTree<T>>.flattened(structureParsers: Set<Parser<*>>) = map { it.flatten(structureParsers) }

/** Performs the same operation as [flatten], using the parsers defined in [grammar] as `structureParsers`. */
fun <T> SyntaxTree<T>.flattenForGrammar(grammar: Grammar<*>) = flatten(grammar.declaredParsers)

/** Performs the same as [flattened], using the parsers defined in [grammar] as `structureParsers`. */
fun <T> Parser<SyntaxTree<T>>.flattenedForGrammar(grammar: Grammar<*>) = map { it.flattenForGrammar(grammar) }
